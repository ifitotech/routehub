...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!email || !companyName || !branchName) return new Response(JSON.stringify({error: 'Missing invite details'}), {status: 400, headers: {...cors, 'content-type': 'application/json'}})
    ...
    return new Response(JSON.stringify({error: error instanceof Error ? error.message : 'Unable to send invitation'}), {status: 500, headers: {...cors, 'content-type': 'application/json'}})
  }
})
