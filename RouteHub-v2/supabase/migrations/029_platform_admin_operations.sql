...
create or replace function public.platform_create_branch(company_id uuid, branch_name text, branch_address text, manager_email text)
returns uuid language plpgsql security definer set search_path = public as $$
...
begin
  ...
  insert into public.branches(company_id, name, address) values (company_id, trim(branch_name), nullif(trim(branch_address), '')) returning id into new_branch_id;
  if nullif(trim(manager_email), '') is not null then
    insert into public.invitations(company_id, branch_id, email, role, status, created_by)
