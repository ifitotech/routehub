-- Permit a driver to close their own reported issue after resolving it, while
-- preserving the existing restriction to progress, GPS, evidence and notes.
...
begin
  ...
  select exists (
    ...
    where cu.user_id = actor and cu.company_id = old.company_id
  ...
  if (to_jsonb(new) - allowed_columns) is distinct from (to_jsonb(old) - allowed_columns) then
    raise exception 'Drivers may only update route progress, evidence, GPS, and issue notes.' using errcode = '42501';
  end if;
  if new.status is distinct from old.status and not (
    ...
    raise exception 'Invalid driver route status transition: % to %.', old.status, new.status using errcode = '23514';
  end if;
  return new;
end;
$$;
