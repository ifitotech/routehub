-- RouteHub: an assigned stop is executable work. The Driver UI already treats
-- assigned as startable, so keep the database transition in sync with that
-- authoritative queue contract. This only expands the existing assigned-user
-- progress transition; it does not change row visibility or dispatch fields.
...
begin
  ...
  if new.status is distinct from old.status and not (
    (old.status in ('draft','pending','published','assigned') and new.status = 'active')
