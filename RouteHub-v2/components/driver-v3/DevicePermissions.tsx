...
export default function DevicePermissions({locale}: {locale: string}) {
  ...
  const requestNotifications = async () => {
    setBusy(true)
    setError('')
    try { await registerPushNotifications(); setNotifications('granted') }
    catch (cause) { setError(cause instanceof Error ? cause.message : (es ? 'No se pudieron activar las notificaciones.' : 'Notifications could not be enabled.')) }
    finally { setBusy(false) }
  }
  return <section className={styles.section}>
    ...
    <div className={styles.row}>
      <span className={styles.rowCopy}><strong>{es ? 'Notificaciones' : 'Notifications'}</strong>
