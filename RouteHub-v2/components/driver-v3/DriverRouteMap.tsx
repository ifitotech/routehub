...
function LiveDriverRouteMap({route, driverFix, locale = 'en'}: {
  ...
  useEffect(() => {
    ...
    const layout = () => {
      ...
      const fadeStart = estimate ? Math.max(0, estimate.getBoundingClientRect().top - rect.top - 120) : detailsStart + 120
...
function StaticDriverRouteMap({route, driverFix, locale = 'en'}: {
  ...
  useEffect(() => {
    ...
    const layout = () => {
      ...
      const height = Math.ceil(estimate.getBoundingClientRect().top - rect.top)
