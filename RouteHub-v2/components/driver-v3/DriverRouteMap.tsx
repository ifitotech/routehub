...
function LiveDriverRouteMap({route, driverFix, locale = 'en'}: {
  ...
  useEffect(() => {
    ...
    const layout = () => {
      ...
      const fadeStart = estimate ? estimate.getBoundingClientRect().top - rect.top - 8 : detailsStart + 120
...
function StaticDriverRouteMap({route, driverFix, locale = 'en'}: {
  ...
  useEffect(() => {
    ...
    const layout = () => {
      ...
      const height = Math.ceil(estimate.getBoundingClientRect().bottom - rect.top + 10)
