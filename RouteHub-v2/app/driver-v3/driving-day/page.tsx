...
export default function DriverV3DrivingDay(){
 ...
 return <DriverV3Shell active="more"><Link href="/driver-v3/more" className="muted">‹ More</Link><p className="eyebrow">WORK</p><h1 className="title">Driving Day</h1>{loading?<section className="card"><p>Loading driving day…</p></section>:error?<section className="card"><p role="alert">{error}</p></section>:<section className="card"><p className="eyebrow">STATUS</p><h2>{drivingSession?'ACTIVE':'NOT STARTED'}</h2>{drivingSession&&<p className="muted">Started {new Date(drivingSession.started_at).toLocaleTimeString([], {hour:'numeric',minute:'2-digit'})}</p>}<button className={drivingSession?'danger':'primary'} disabled={busy||!driverId||!companyId} onClick={()=>void toggle()}>{busy?'Updating…':drivingSession?'END DRIVING DAY':'START DRIVING DAY'}</button>{message&&<p role="status" className="muted">{message}</p>}</section>}</DriverV3Shell>
}
