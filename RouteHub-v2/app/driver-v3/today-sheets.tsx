...
export function DeliverySheet({
  ...
  return (
    <div className={`${styles.completionOverlay} ${styles.todaySheetOverlay}`} style={{...overlay, top: 'calc(56px + env(safe-area-inset-top))', alignItems: 'stretch', padding: 0, background: 'var(--today-surface, #101c2d)'}} onTouchMove={e => e.preventDefault()} onClick={onClose}>
      <section className={`card ${styles.completionDialog} ${styles.todaySheetPanel}`} style={{...dialog, width: '100%', maxWidth: 680, minHeight: '100%', borderRadius: '24px 24px 0 0', padding: '20px 18px calc(20px + env(safe-area-inset-bottom))'}} onClick={e => e.stopPropagation()}>
