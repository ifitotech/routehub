...
import {MapPin, MoreHorizontal, PackagePlus, Pencil, Phone, Plus, Search, Trash2, UserRound, X} from 'lucide-react'
...
export default function Contacts() {
  ...
  const copy = {
    en:{edit:'Edit',remove:'Delete',pickup:'Pickup',delivery:'Delivery',map:'Map',more:'More actions',editTitle:'Edit contact',updated:'Contact updated.',deleted:'Contact deleted.',confirm:'Delete contact?',confirmHelp:'This contact will be permanently removed.',keep:'Keep contact'},
    es:{edit:'Editar',remove:'Eliminar',pickup:'Recogida',delivery:'Entrega',map:'Mapa',more:'Más acciones',editTitle:'Editar contacto',updated:'Contacto actualizado.',deleted:'Contacto eliminado.',confirm:'¿Eliminar contacto?',confirmHelp:'Este contacto se eliminará permanentemente.',keep:'Conservar contacto'},
    fr:{edit:'Modifier',remove:'Supprimer',pickup:'Collecte',delivery:'Livraison',map:'Carte',more:'Plus d’actions',editTitle:'Modifier le contact',updated:'Contact mis à jour.',deleted:'Contact supprimé.',confirm:'Supprimer le contact ?',confirmHelp:'Ce contact sera définitivement supprimé.',keep:'Conserver le contact'},
  }[locale]
  ...
  return <ManagerShell active="contacts" branchName={t.mainBranch} displayName={t.managerRole} roleLabel={locale === 'es' ? 'Manager de sucursal' : locale === 'fr' ? 'Manager de succursale' : 'Branch Manager'}><div className={styles.page}>
    ...
    <section className={styles.list} aria-label={t.contacts}>{filtered.map(contact => <article className={styles.contactCard} key={contact.id}><div className={styles.avatar} aria-hidden="true">{contact.company_name.slice(0, 2)}</div><div className={styles.identity}><div className={styles.identityHeader}><div><h2>{contact.company_name}</h2><p className={styles.person}>{contact.contact_name || t.contactPerson}</p></div></div><p className={styles.address}>{contact.address}</p>{contact.phone && <p className={styles.phone}><Phone size={14}/>{contact.phone}</p>}<div className={styles.cardActions}><Link className={styles.pickupAction} href={`/routes?contact=${encodeURIComponent(contact.id)}&type=pickup`} title={copy.pickup}><PackagePlus size={16}/><span>{copy.pickup}</span></Link><Link className={styles.deliveryAction} href={`/routes?contact=${encodeURIComponent(contact.id)}&type=delivery`} title={copy.delivery}><PackagePlus size={16}/><span>{copy.delivery}</span></Link><div className={styles.moreWrap}><button type="button" className={styles.moreButton} aria-haspopup="menu" aria-expanded={menuId === contact.id} aria-label={copy.more} onClick={() => setMenuId(current => current === contact.id ? null : contact.id)}><MoreHorizontal size={16}/></button>{menuId === contact.id && <><div className={styles.menuBackdrop} onClick={() => setMenuId(null)}/><div className={styles.menu} role="menu"><a role="menuitem" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(contact.address)}`} target="_blank" rel="noreferrer" onClick={() => setMenuId(null)}><MapPin size={15}/>{copy.map}</a>{contact.phone && <a role="menuitem" href={`tel:${contact.phone}`} onClick={() => setMenuId(null)}><Phone size={15}/>{t.call}</a>}{canManage && <><button role="menuitem" type="button" onClick={() => { edit(contact); setMenuId(null) }}><Pencil size={15}/>{copy.edit}</button><button role="menuitem" type="button" className={styles.dangerItem} onClick={() => { setDeleteTarget(contact); setMenuId(null) }}><Trash2 size={15}/>{copy.remove}</button></>}</div></>}</div></div></div></article>)}{!filtered.length && <section className={styles.empty}><span className={styles.emptyIcon}><UserRound size={24}/></span><h2>{t.noContacts}</h2><p>{query ? t.tryAnotherContact : t.addFirstContact}</p></section>}</section>
