...
const [recipientError,setRecipientError]=useState('')
  ...
  const [routeView,setRouteView]=useState<'queue'|'details'|'truck'|null>(()=>searchParams.get('view')==='truck'?'truck':searchParams.get('view')==='route'?'queue':null)
  ...
  const returnToToday=()=>{
    if((routeView==='queue'||routeView==='truck')&&typeof document!=='undefined'&&(document as any).startViewTransition){runMapTransition(()=>setRouteView(null));return}
    setRouteView(null)
  }
  ...
  return <main className={`app ${styles.page}${gestureActive?` ${styles.gestureActive}`:''}${routeView!==null?` ${styles.routeMode}`:''}`} onPointerDown={routeView===null?beginTodayDrag:undefined} onPointerMove={routeView===null?moveTodayDrag:undefined} onPointerUp={routeView===null?finishTodayDrag:undefined} onPointerCancel={()=>{if(routeView===null){todayDragStart.current=null;setGestureActive(false);setTodayDragY(0)}}} onTouchStart={routeView===null?beginTodaySwipe:undefined} onTouchMove={routeView===null?moveTodaySwipe:undefined} onTouchEnd={routeView===null?finishTodaySwipe:undefined} onTouchCancel={()=>{routeTouchStart.current=null;setGestureActive(false);setTodayDragY(0)}}>
    ...
    {loading&&!missions.length?<section className={`${styles.loading} card`} aria-busy="true"><span/><span/><span/></section>:current?<>
      <section className={`${styles.routeHero}${routeView==='truck'||routeView==='queue'?` ${styles.routeHeroMapOpen}`:''}`} style={{'--today-drag':`${todayDragY}px`,'--today-progress':`${Math.min(1,Math.abs(todayDragY)/150)}`,'--today-opacity':`${1-Math.min(1,Math.abs(todayDragY)/150)}`,'--today-blur':`${Math.round(Math.min(1,Math.abs(todayDragY)/150)*6)}px`} as CSSProperties}>
      ...
      <div className={`${styles.primaryActions}${routeView==='truck'||routeView==='queue'?` ${styles.primaryActionsMapOpen}`:''}`} style={{'--today-drag':`${todayDragY}px`,'--today-progress':`${Math.min(1,Math.abs(todayDragY)/150)}`,'--today-opacity':`${1-Math.min(1,Math.abs(todayDragY)/150)}`,'--today-blur':`${Math.round(Math.min(1,Math.abs(todayDragY)/150)*6)}px`} as CSSProperties}>
