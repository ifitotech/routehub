...
import {Camera, Check, ChevronRight, CircleUserRound, ClipboardCheck, Clock3, FileText, History as HistoryIcon, Home, List, MapPin, MessageSquare, MoreHorizontal, Pause, Phone, Play, RotateCcw, Signature, Truck, TriangleAlert, X} from 'lucide-react'
...
type TruckSession = {id:string;name:string;make?:string|null;model?:string|null;plate_number?:string|null;current_odometer?:number|null}
type TruckFuelLog = {id:string;filled_at:string;odometer?:number|null;amount?:number|null}
type TruckMaintenanceLog = {id:string;serviced_at:string;maintenance_type:string;odometer?:number|null;amount?:number|null}
...
const [recipientError,setRecipientError]=useState('')
  ...
  const [truckSession,setTruckSession]=useState<TruckSession|null>(null)
  const [truckFuelLogs,setTruckFuelLogs]=useState<TruckFuelLog[]>([])
  const [truckMaintenanceLogs,setTruckMaintenanceLogs]=useState<TruckMaintenanceLog[]>([])
  const [truckCompanyId,setTruckCompanyId]=useState('')
  const [truckBranchId,setTruckBranchId]=useState('')
  const [truckForm,setTruckForm]=useState<'fuel'|'maintenance'|null>(null)
  ...
  const [routeCompletedNotice,setRouteCompletedNotice]=useState(false)
  const [completedStopNotice,setCompletedStopNotice]=useState<Mission|null>(null)
  ...
  const [routeView,setRouteView]=useState<'queue'|'details'|'truck'|'map'|null>(()=>searchParams.get('view')==='truck'?'truck':searchParams.get('view')==='map'?'map':searchParams.get('view')==='route'?'queue':null)
  ...
  const load=useCallback(async(preserveMessage=true)=>{
    setLoading(true)
    try{
      ...
      const {data:truckData}=await client.from('trucks').select('id,name,make,model,plate_number,current_odometer').eq('company_id',membership.company_id).eq('branch_id',membership.branch_id).eq('active',true).maybeSingle()
      setTruckCompanyId(membership.company_id);setTruckBranchId(membership.branch_id||'')
      setTruckSession((truckData||null) as TruckSession|null)
      if(truckData){
        const [fuelResult,maintenanceResult]=await Promise.all([
          client.from('truck_fuel_logs').select('id,filled_at,odometer,amount').eq('truck_id',truckData.id).order('filled_at',{ascending:false}).limit(3),
          client.from('truck_maintenance_logs').select('id,serviced_at,maintenance_type,odometer,amount').eq('truck_id',truckData.id).order('serviced_at',{ascending:false}).limit(3),
        ])
        setTruckFuelLogs((fuelResult.data||[]) as TruckFuelLog[])
        setTruckMaintenanceLogs((maintenanceResult.data||[]) as TruckMaintenanceLog[])
      }else{
        setTruckFuelLogs([])
        setTruckMaintenanceLogs([])
      }
  ...
  const completionQueue=missions.filter(route=>route.driver_id===driverId&&(route.route_date||'').slice(0,10)<=today).reduce<Mission[][]>((groups,route)=>{
    ...
    .sort((left,right)=>(left.items[0]?.route_date||'').localeCompare(right.items[0]?.route_date||''))
  ...
  const routeFocusDate=(current?.route_date||today).slice(0,10)
  const dayRoutes=missions.filter(route=>route.driver_id===driverId&&(route.route_date||'').slice(0,10)===routeFocusDate&&route.status!=='cancelled'&&(!current||(route.company_id===current.company_id&&route.branch_id===current.branch_id))).slice().sort((left,right)=>{
  ...
  const routeList=missions.filter(route=>{
    ...
    // Keep the driver's current day complete, while retaining only unresolved
    // work from earlier days so a pending or issue stop can still be fixed.
    return routeDate===today || (routeDate<today&&['published','pending','issue'].includes(route.status))
  }).slice().sort((left,right)=>{
    const dateOrder=(left.route_date||'').localeCompare(right.route_date||'')
    if(dateOrder!==0)return dateOrder
    const rank=(status:string)=>status==='active'||status==='paused'?0:status==='published'||status==='pending'?1:status==='issue'?2:3
    return rank(left.status)-rank(right.status)||left.position-right.position||left.id.localeCompare(right.id)
  })
  const routePlanCompleted=routeList.filter(route=>route.status==='completed').length
  const routePlanRemaining=Math.max(0,routeList.length-routePlanCompleted)
  const routePlanNext=routeList.find(route=>!['completed','cancelled'].includes(route.status))
  const completedNextStop=completedStopNotice?missions.filter(route=>route.id!==completedStopNotice.id&&route.driver_id===driverId&&route.company_id===completedStopNotice.company_id&&route.branch_id===completedStopNotice.branch_id&&route.route_date===completedStopNotice.route_date&&!['completed','cancelled'].includes(route.status)).sort((left,right)=>left.position-right.position||left.id.localeCompare(right.id))[0]:null
  const dayMapOrigin=current?.origin_address||dayRoutes[0]?.origin_address
  const dayMapWaypoints=dayRoutes.slice(0,-1).map(route=>({address:route.destination_address,label:route.destination_name||route.destination_address,coordinate:route.destination_lat!=null&&route.destination_lng!=null?{lat:Number(route.destination_lat),lng:Number(route.destination_lng)}:null}))
  const dayMapDestination=dayRoutes[dayRoutes.length-1]
  ...
  const isPendingRoute=Boolean(current&&['published','pending'].includes(current.status))
  const pastDueCopy=locale==='es'?'ATRASADA · PENDIENTE':locale==='fr'?'EN RETARD · EN ATTENTE':'PAST DUE · PENDING'
  const pendingCopy=locale==='es'?'PENDIENTE':locale==='fr'?'EN ATTENTE':'PENDING'
  ...
  // This is a planning/reference map only. It never starts the in-app GPS
  // navigator; driving directions are handed off to Google Maps.
  const openTruck=()=>{
    if(routeView==='truck')return
    runMapTransition(()=>{setTodayDragY(0);setRouteView('truck')})
  }
  const openMap=()=>{if(routeView==='map')return;setTodayDragY(0);setRouteView('map')}
  const openRouteQueue=()=>{setTodayDragY(0);setRouteView('queue')}
  const returnToToday=()=>{
  ...
  const beginRouteSwipe=(event:React.TouchEvent<HTMLElement>)=>{
    if(event.target instanceof Element&&event.target.closest('button,a,input,textarea,select'))return
    const touch=event.touches[0]
    if(touch){routeTouchStart.current={x:touch.clientX,y:touch.clientY};setGestureActive(true)}
  }
  ...
  const detailCopy=locale==='es'
    ? {stopDetails:'Detalles de la parada',current:'Parada actual',upcoming:'Próxima parada',completed:'Completada',issueStatus:'Incidencia',completion:'Detalles de finalización',completedAt:'Completada',arrivedAt:'Llegada',time:'Tiempo en ruta',contact:'Contacto',instructions:'Instrucciones',po:'PO / ORDER',photo:'Foto disponible',signature:'Firma disponible',packing:'Packing list disponible',issue:'Incidencia reportada',receivedBy:'Recibido por',openMap:'Navegar',close:'Cerrar'}
    : locale==='fr'
      ? {stopDetails:'Détails de l’arrêt',current:'Arrêt actuel',upcoming:'Prochain arrêt',completed:'Terminé',issueStatus:'Incident',completion:'Détails de fin',completedAt:'Terminé le',arrivedAt:'Arrivée',time:'Temps sur l’itinéraire',contact:'Contact',instructions:'Instructions',po:'PO / COMMANDE',photo:'Photo disponible',signature:'Signature disponible',packing:'Liste de colisage disponible',issue:'Incident signalé',receivedBy:'Reçu par',openMap:'Ouvrir la carte',close:'Fermer'}
      : {stopDetails:'Stop details',current:'Current stop',upcoming:'Upcoming stop',completed:'Completed',issueStatus:'Issue',completion:'Completion details',completedAt:'Completed',arrivedAt:'Arrival',time:'Time on route',contact:'Contact',instructions:'Instructions',po:'PO / ORDER',photo:'Photo available',signature:'Signature available',packing:'Packing list available',issue:'Issue reported',receivedBy:'Received by',openMap:'Navigate',close:'Close'}
  ...
  const startRoute=async()=>{
    ...
    // Google Maps is the sole turn-by-turn navigator during the beta. RouteHub
    // keeps the operational stop open so the driver can return to record proof.
  ...
  return <main className={`app ${styles.page}${gestureActive?` ${styles.gestureActive}`:''}${routeView!==null?` ${styles.routeMode}`:''}`} onPointerDown={routeView===null?beginTodayDrag:undefined} onPointerMove={routeView===null?moveTodayDrag:undefined} onPointerUp={routeView===null?finishTodayDrag:undefined} onPointerCancel={()=>{if(routeView===null){todayDragStart.current=null;setGestureActive(false);setTodayDragY(0)}}} onTouchStart={routeView===null?beginTodaySwipe:undefined} onTouchMove={routeView===null?moveTodaySwipe:undefined} onTouchEnd={routeView===null?finishTodaySwipe:undefined} onTouchCancel={()=>{routeTouchStart.current=null;setGestureActive(false);setTodayDragY(0)}}>
    <header className={styles.header}><div className={styles.brand}><Image src="/routehub-driver-new.jpg" alt="RouteHub Driver" width={48} height={48} priority/><strong>RouteHub</strong></div></header><div className={styles.workspaceHeading}><span className={styles.workspace}>{t.driverWorkspace}</span><h1>{routeView==='truck'?'Truck':routeView===null?t.routes:'Route'}</h1></div>
    ...
    {loading&&!missions.length?<section className={`${styles.loading} card`} aria-busy="true"><span/><span/><span/></section>:current?<>
      <section className={`${styles.routeHero}${routeView==='truck'||routeView==='queue'||routeView==='map'?` ${styles.routeHeroMapOpen}`:''}`} style={{'--today-drag':`${todayDragY}px`,'--today-progress':`${Math.min(1,Math.abs(todayDragY)/150)}`,'--today-opacity':`${1-Math.min(1,Math.abs(todayDragY)/150)}`,'--today-blur':`${Math.round(Math.min(1,Math.abs(todayDragY)/150)*6)}px`} as CSSProperties}>
      <section className={styles.mission}>
        <div className={styles.missionTop}><span className={isPastRoute?styles.pastDueLabel:undefined}>{isPastRoute?(locale==='es'?'ATRASADA':'PAST DUE'):t.currentRoute}</span><span className={`${current.priority==='urgent'?styles.urgent:styles.priority}${isPastRoute||isPendingRoute?` ${styles.pastDuePending}`:''}`}>{isPastRoute?(locale==='es'?'PENDIENTE':'PENDING'):isPendingRoute?pendingCopy:current.priority==='urgent'?`⚠ ${t.urgent}`:current.priority||t.normal}</span></div>
      ...
      <LiveRouteMap originAddress={current.origin_address} destinationAddress={current.destination_address} originCoordinate={current.origin_lat!=null&&current.origin_lng!=null?{lat:Number(current.origin_lat),lng:Number(current.origin_lng)}:null} destinationCoordinate={current.destination_lat!=null&&current.destination_lng!=null?{lat:Number(current.destination_lat),lng:Number(current.destination_lng)}:null} driverLocation={drivingSession?.last_lat!=null&&drivingSession.last_lng!=null?{lat:drivingSession.last_lat,lng:drivingSession.last_lng}:null} driverUpdatedAt={drivingSession?.last_updated_at} title="Ruta de referencia" showHeader={false} showLocationUpdated={false} interactive={false} onActivate={()=>openGoogleMaps(current)} useDriverAsOrigin locale={locale}/>
      </section>
      <div className={`${styles.primaryActions}${routeView==='truck'||routeView==='queue'||routeView==='map'?` ${styles.primaryActionsMapOpen}`:''}`} style={{'--today-drag':`${todayDragY}px`,'--today-progress':`${Math.min(1,Math.abs(todayDragY)/150)}`,'--today-opacity':`${1-Math.min(1,Math.abs(todayDragY)/150)}`,'--today-blur':`${Math.round(Math.min(1,Math.abs(todayDragY)/150)*6)}px`} as CSSProperties}>
        ...
        {current.status==='active'&&(currentKind==='delivery'||currentAction!=='arrived')&&<button disabled={busy} className={styles.complete} onClick={()=>{if(currentKind==='delivery')setDeliveryToolsOpen(true);else void completeCurrentStop()}}><Check size={19}/>{currentKind==='delivery'?stopCopy.completeDelivery:currentAction==='confirm_pickup'?stopCopy.confirmPickup:stopCopy.completeBranch}</button>}
         {current.destination_address&&<button disabled={busy} className={`${styles.viewRoute} ${styles.googleMapsButton}`} onClick={()=>openGoogleMaps(current)}><MapPin size={22}/><span>{stopCopy.openMaps}</span><ChevronRight className={styles.googleMapsChevron} size={20} aria-hidden="true"/></button>}
      ...
      {routeView===null&&<>
        {upcoming[0]&&<button type="button" className={styles.todayNextStop} onClick={openRouteQueue}>
          <span className={styles.todayNextStopLabel}>{locale==='es'?'PRÓXIMA PARADA':locale==='fr'?'PROCHAIN ARRÊT':'NEXT STOP'}</span>
          <span className={styles.todayNextStopBody}>
            <span className={styles.todayNextStopNumber}>{upcoming[0].position}</span>
            <span className={styles.todayNextStopText}><strong>{routeLabel(upcoming[0])}</strong><small>{stopKind(upcoming[0].mission_type)==='branch'?stopCopy.branch:stopKind(upcoming[0].mission_type).toUpperCase()} · {upcoming[0].destination_address||t.destination}</small>{upcoming[0].order_number&&<small>{routeMetaCopy.po}: {upcoming[0].order_number}</small>}</span>
            <ChevronRight size={20} aria-hidden="true" />
          </span>
        </button>}
        <section className={styles.todayQuickActions} aria-label={locale==='es'?'Acciones rápidas':'Quick actions'}>
          <button type="button" onClick={openRouteQueue}><List size={21}/><span>{locale==='es'?'Ruta':'Route'}</span></button>
          <button type="button" onClick={()=>openGoogleMaps(current)} disabled={!current.destination_address}><MapPin size={21}/><span>{locale==='es'?'Mapa':'Map'}</span></button>
          <button type="button" onClick={()=>setModal(true)}><TriangleAlert size={21}/><span>{locale==='es'?'Reportar problema':'Report issue'}</span></button>
        </section>
      </>}
      ...
      {routeView==='map'&&current&&<section className={styles.activeMapPage} aria-label={locale==='es'?'Mapa de ruta':'Active route map'}><div className={styles.activeMapCanvas}><LiveRouteMap originAddress={current.origin_address} destinationAddress={current.destination_address} originCoordinate={current.origin_lat!=null&&current.origin_lng!=null?{lat:Number(current.origin_lat),lng:Number(current.origin_lng)}:null} destinationCoordinate={current.destination_lat!=null&&current.destination_lng!=null?{lat:Number(current.destination_lat),lng:Number(current.destination_lng)}:null} driverLocation={drivingSession?.last_lat!=null&&drivingSession.last_lng!=null?{lat:drivingSession.last_lat,lng:drivingSession.last_lng}:null} driverUpdatedAt={drivingSession?.last_updated_at} showHeader={false} interactive title="Active route" useDriverAsOrigin locale={locale}/></div><div className={styles.activeMapSheet}><span className={styles.activeMapEyebrow}>{locale==='es'?'PARADA ACTUAL':'CURRENT STOP'}</span><strong>{currentStopLabel} · {currentStopIndex>=0?`${currentStopIndex+1} / ${currentRouteStops.length}`:''}</strong><h2>{routeLabel(current)}</h2><p>{current.destination_address||t.destination}</p>{current.order_number&&<small>{routeMetaCopy.po}: {current.order_number}</small>}<button type="button" className={styles.mapPrimaryAction} onClick={()=>openGoogleMaps(current)}><MapPin size={20}/>{stopCopy.openMaps}</button>{current.status==='active'&&!current.arrived_at&&<button type="button" className={styles.mapArrivedAction} disabled={busy} onClick={()=>void markArrived()}><Check size={20}/>{stopCopy.arrived}</button>}</div></section>}
      {completedStopNotice&&<section className={styles.stopCompletedPanel} role="status"><div className={styles.stopCompletedIcon}><Check size={34}/></div><p className={styles.stopCompletedEyebrow}>{stopKind(completedStopNotice.mission_type)==='pickup'?'PICKUP':stopKind(completedStopNotice.mission_type)==='delivery'?'DELIVERY':stopCopy.branch} {locale==='es'?'COMPLETADO':'COMPLETED'}</p><h2>{locale==='es'?'Parada completada':locale==='fr'?'Arrêt terminé':'Stop completed'}</h2><p className={styles.stopCompletedSub}>{routeLabel(completedStopNotice)}</p><p className={styles.stopCompletedAddress}>{completedStopNotice.destination_address||t.destination}</p>{completedStopNotice.completed_at&&<small>{new Date(completedStopNotice.completed_at).toLocaleTimeString(locale,{hour:'numeric',minute:'2-digit'})}</small>}{completedNextStop&&<div className={styles.stopCompletedNext}><b>{locale==='es'?'PRÓXIMA PARADA':'UP NEXT'}</b><strong>#{completedNextStop.position} · {routeLabel(completedNextStop)}</strong><span>{stopKind(completedNextStop.mission_type)==='branch'?stopCopy.branch:stopKind(completedNextStop.mission_type).toUpperCase()} · {completedNextStop.destination_address||t.destination}</span>{completedNextStop.order_number&&<span>{routeMetaCopy.po}: {completedNextStop.order_number}</span>}</div>}<div className={styles.stopCompletedActions}>{completedNextStop?<button type="button" className={styles.completeContinue} onClick={()=>{setCompletedStopNotice(null);setMessage('');void load()}}><ChevronRight size={20}/>{locale==='es'?'Continuar a la próxima parada':'Continue to next stop'}</button>:<p className={styles.stopCompletedFinal}>{locale==='es'?'Todas las paradas requeridas están completadas.':'All required stops are complete.'}</p>}<button type="button" className={styles.completeViewRoute} onClick={openRouteQueue}>{locale==='es'?'Ver ruta':'View route'}</button></div></section>}
      {routeView==='truck'&&truckSession&&<section className={styles.truckForms}><div className={styles.truckQuickActions}><button type="button" className={truckForm==='fuel'?styles.truckQuickActive:undefined} onClick={()=>setTruckForm(truckForm==='fuel'?null:'fuel')}><span>⛽</span>Log fuel</button><button type="button" className={truckForm==='maintenance'?styles.truckQuickActive:undefined} onClick={()=>setTruckForm(truckForm==='maintenance'?null:'maintenance')}><span>🔧</span>Maintenance</button></div>{truckMessage&&<p className={styles.truckFormMessage} role="status">{truckMessage}</p>}{truckForm==='fuel'&&<form className={styles.truckForm} onSubmit={event=>{event.preventDefault();void saveTruckFuel()}}><h3>Log fuel</h3><label>Mileage<input type="number" inputMode="decimal" min="0" step="0.1" value={fuelOdometer} onChange={event=>setFuelOdometer(event.target.value)} required/></label><label>Fuel amount / cost<input type="number" inputMode="decimal" min="0" step="0.01" value={fuelAmount} onChange={event=>setFuelAmount(event.target.value)} required/></label><label className={styles.truckPhotoInput}>Receipt photo (optional)<input type="file" accept="image/*" capture="environment" onChange={event=>setFuelPhoto(event.target.files?.[0]||null)}/></label><button type="submit" disabled={truckSaving}>{truckSaving?'Saving…':'Save fuel'}</button></form>}{truckForm==='maintenance'&&<form className={styles.truckForm} onSubmit={event=>{event.preventDefault();void saveTruckMaintenance()}}><h3>Log maintenance</h3><label>Maintenance type<input value={maintenanceType} onChange={event=>setMaintenanceType(event.target.value)} required/></label><label>Mileage<input type="number" inputMode="decimal" min="0" step="0.1" value={maintenanceOdometer} onChange={event=>setMaintenanceOdometer(event.target.value)} required/></label><label>Cost (optional)<input type="number" inputMode="decimal" min="0" step="0.01" value={maintenanceAmount} onChange={event=>setMaintenanceAmount(event.target.value)}/></label><label className={styles.truckPhotoInput}>Receipt photo (optional)<input type="file" accept="image/*" capture="environment" onChange={event=>setMaintenancePhoto(event.target.files?.[0]||null)}/></label><button type="submit" disabled={truckSaving}>{truckSaving?'Saving…':'Save maintenance'}</button></form>}</section>}
      {routeView==='queue'&&routeList.length>0&&<aside className={styles.routeListSummary} aria-label="Route progress"><div><strong>{routeList.length} {locale==='es'?'paradas':'stops'}</strong><span><b>{routePlanCompleted}</b> {locale==='es'?'completadas':'completed'} · <b>{routePlanRemaining}</b> {locale==='es'?'restantes':'remaining'}</span></div><div className={styles.routeProgressTrack}><i style={{width:`${Math.round(routePlanCompleted/routeList.length*100)}%`}}/></div></aside>}
      ...
      {routeView&&<section className={`${styles.routeOverlay}${routeView==='queue'?` ${styles.routeOverlayQueue}`:''}`} style={routeView==='queue'?{'--route-drag':`${todayDragY}px`} as CSSProperties:undefined} aria-label="Route details" onPointerDown={routeView==='queue'?beginTodayDrag:undefined} onPointerMove={routeView==='queue'?moveTodayDrag:undefined} onPointerUp={routeView==='queue'?finishTodayDrag:undefined} onPointerCancel={()=>{if(routeView==='queue'){todayDragStart.current=null;setGestureActive(false);setTodayDragY(0)}}} onTouchStart={routeView==='queue'?beginRouteSwipe:undefined} onTouchMove={routeView==='queue'?moveRouteSwipe:undefined} onTouchEnd={routeView==='queue'?finishRouteSwipe:undefined} onTouchCancel={()=>{routeTouchStart.current=null;setGestureActive(false);setTodayDragY(0)}}>
      {routeView==='queue'?<div className={styles.stopList}>{!routeList.length?<div className={styles.empty}><List size={24}/><h2>{t.noRoutesToday}</h2><p>{t.createRouteWhenReady}</p></div>:routeList.map((route,index)=>{const kind=stopKind(route.mission_type);const pastDue=(route.route_date||'').slice(0,10)<today&&!['completed','issue','cancelled'].includes(route.status);const isIssue=route.status==='issue';const isPending=['published','pending'].includes(route.status);const isCompleted=route.status==='completed';const status=pastDue?pastDueCopy:isIssue?detailCopy.issueStatus:isPending?pendingCopy:route.status==='completed'?detailCopy.completed:route.status==='active'||route.status==='paused'?detailCopy.current:route.scheduled_at?new Date(route.scheduled_at).toLocaleTimeString(locale,{hour:'numeric',minute:'2-digit'}):detailCopy.upcoming;const rowClass=`${styles.stopRow}${pastDue||isPending?` ${styles.stopRowPastDue}`:''}${isIssue?` ${styles.stopRowIssue}`:''}`;return <button type="button" className={rowClass} key={route.id} onClick={()=>{setSelectedRouteId(route.id);setRouteView('details')}}><span className={isCompleted?`${styles.stopNumber} ${styles.stopNumberCompleted}`:isIssue?`${styles.stopNumber} ${styles.stopNumberIssue}`:styles.stopNumber}>{index+1}</span><span><strong>{routeLabel(route)}</strong><small className={isIssue?styles.issueStatus:isPending?styles.pendingStatus:isCompleted?styles.completedStatus:undefined}>{kind==='branch'?stopCopy.branch:kind.toUpperCase()} · {status}{route.status==='active'&&elapsedLabel(route)?` · ${elapsedLabel(route)}`:''}</small>{kind==='pickup'&&route.order_number&&<small className={styles.stopPo}>{routeMetaCopy.po}: {route.order_number}</small>}</span><ChevronRight size={18}/></button>})}</div>:routeView==='truck'?<div className={styles.truckPage}>{truckSession?<><section className={styles.truckHero}><div><p className={styles.truckEyebrow}>TRUCK SESSION</p><h2>{truckSession.name}</h2><p>{[truckSession.make,truckSession.model].filter(Boolean).join(' ')||'Branch vehicle'}{truckSession.plate_number?` · ${truckSession.plate_number}`:''}</p></div><div className={styles.truckOdometer}><span>Odometer</span><strong>{truckSession.current_odometer??'—'}</strong></div></section><div className={styles.truckGrid}><section className={styles.truckPanel}><div className={styles.sectionTitle}><span>Fuel</span><b>{truckFuelLogs.length}</b></div>{truckFuelLogs.length?truckFuelLogs.map(log=><div className={styles.truckRow} key={log.id}><div><strong>{new Date(log.filled_at).toLocaleDateString(locale,{month:'short',day:'numeric'})}</strong><span>{log.odometer!=null?`Odometer ${log.odometer}`:'Odometer —'}</span></div><strong>{log.amount!=null?`$${Number(log.amount).toFixed(2)}`:'—'}</strong></div>):<p className={styles.truckMuted}>No fuel logs yet.</p>}</section><section className={styles.truckPanel}><div className={styles.sectionTitle}><span>Maintenance</span><b>{truckMaintenanceLogs.length}</b></div>{truckMaintenanceLogs.length?truckMaintenanceLogs.map(log=><div className={styles.truckRow} key={log.id}><div><strong>{log.maintenance_type}</strong><span>{new Date(log.serviced_at).toLocaleDateString(locale,{month:'short',day:'numeric'})}</span></div><strong>{log.amount!=null?`$${Number(log.amount).toFixed(2)}`:'—'}</strong></div>):<p className={styles.truckMuted}>No maintenance logs yet.</p>}</section></div></>:<div className={styles.empty}><Truck size={24}/><h2>No truck assigned</h2><p>Assign a truck for this branch to start tracking fuel and maintenance.</p></div>}</div>:selectedRoute&&<div className={styles.stopDetails}><span className={styles.stopNumber}>{selectedRoute.position}</span><h2>{routeLabel(selectedRoute)}</h2><p><MapPin size={17}/>{selectedRoute.destination_address||t.destination}</p><div className={styles.detailDivider}/><small>{stopKind(selectedRoute.mission_type)==='branch'?stopCopy.branch:stopKind(selectedRoute.mission_type).toUpperCase()}</small><strong className={selectedRoute.status==='issue'?styles.issueStatus:undefined}>{selectedRoute.status==='issue'?detailCopy.issueStatus:selectedRoute.status==='active'?detailCopy.current:selectedRoute.status==='completed'?detailCopy.completed:detailCopy.upcoming}</strong>{selectedRoute.scheduled_at&&<p><Clock3 size={17}/>{new Date(selectedRoute.scheduled_at).toLocaleString(locale,{dateStyle:'medium',timeStyle:'short'})}</p>}{selectedRoute.arrived_at&&<p><Clock3 size={17}/>{detailCopy.arrivedAt}: {new Date(selectedRoute.arrived_at).toLocaleString(locale,{dateStyle:'medium',timeStyle:'short'})}</p>}{selectedRoute.destination_address&&<button type="button" className={`${styles.viewRoute} ${styles.googleMapsButton}`} onClick={()=>openGoogleMaps(selectedRoute)}><MapPin size={22}/><span>{stopCopy.openMaps}</span><ChevronRight className={styles.googleMapsChevron} size={20} aria-hidden="true"/></button>}{selectedRoute.order_number&&<div className={styles.detailNotes}><b>{detailCopy.po}</b><span>{selectedRoute.order_number}</span></div>}{receivedBy(selectedRoute)&&<div className={styles.detailNotes}><b>{detailCopy.receivedBy}</b><span>{receivedBy(selectedRoute)}</span></div>}{selectedRoute.destination_phone&&<p><Phone size={17}/>{selectedRoute.destination_phone}</p>}{selectedRoute.notes&&<div className={styles.detailNotes}><b>{detailCopy.instructions}</b><span>{selectedRoute.notes}</span></div>}{selectedRoute.status==='completed'||selectedRoute.status==='issue'?<div className={styles.detailNotes}><b>{selectedRoute.status==='issue'?detailCopy.issue:detailCopy.completion}</b>{selectedRoute.completed_at&&<span><Clock3 size={15}/> {detailCopy.completedAt}: {new Date(selectedRoute.completed_at).toLocaleString(locale,{dateStyle:'medium',timeStyle:'short'})}</span>}{selectedRoute.arrived_at&&selectedRoute.completed_at&&<span><Clock3 size={15}/> {detailCopy.time}: {elapsedLabel(selectedRoute)}</span>}{selectedRoute.finalization_method==='issue'&&<span>{detailCopy.issue}{selectedRoute.finalization_issue?`: ${selectedRoute.finalization_issue}`:''}</span>}{evidencePreview.photo&&<img src={evidencePreview.photo} alt={detailCopy.photo} style={{width:'100%',maxHeight:220,objectFit:'cover',borderRadius:12,marginTop:10}}/>}{evidencePreview.signature&&<img src={evidencePreview.signature} alt={detailCopy.signature} style={{width:'100%',maxHeight:140,objectFit:'contain',background:'#fff',borderRadius:12,marginTop:10}}/>}{selectedEvidence.map((evidence,index)=><span key={`${evidence.kind}-${index}`}>✓ {evidence.kind==='signature'?detailCopy.signature:evidence.kind==='photo'?detailCopy.photo:detailCopy.packing}</span>)}{selectedRoute.driver_note&&!receivedBy(selectedRoute)&&<span><FileText size={15}/> {selectedRoute.driver_note}</span>}{selectedRoute.finalization_note&&<span><FileText size={15}/> {selectedRoute.finalization_note}</span>}</div>:null}</div>}</section>}
...
{recipientPromptOpen&&<div className={styles.backdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="recipient-title"><button className={styles.close} aria-label={t.close} onClick={()=>{if(!busy){setRecipientError('');setRecipientPromptOpen(false)}}}><X/></button><div className={styles.modalIcon}><Check/></div><h2 id="recipient-title">{locale==='es'?'¿Quién recibió la entrega?':locale==='fr'?'Qui a reçu la livraison ?':'Who received the delivery?'}</h2><p>{locale==='es'?'Escribe el nombre de quien recibió. La foto, firma y nota son opcionales.':locale==='fr'?'Saisissez le nom du destinataire. Photo, signature et note sont facultatives.':'Enter the recipient name. Photo, signature and note are optional.'}</p>{recipientError&&<p className={styles.modalError} role="alert">{recipientError}</p>}<input autoFocus value={recipientName} onChange={event=>{setRecipientName(event.target.value);setRecipientError('')}} placeholder={locale==='es'?'Nombre de quien recibió':locale==='fr'?'Nom du destinataire':'Recipient name'} /><button className={styles.photoButton} disabled={!recipientName.trim()||busy} onClick={saveRecipientAndComplete}><Check/>{busy?(locale==='es'?'Guardando…':locale==='fr'?'Enregistrement…':'Saving…'):(locale==='es'?'Guardar y completar':locale==='fr'?'Enregistrer et terminer':'Save and complete')}</button><button className={styles.secondaryButton} disabled={busy} onClick={()=>{setRecipientError('');setRecipientPromptOpen(false)}}>{t.cancel}</button></section></div>}
    ...
    <nav className={styles.driverNav} aria-label="Driver navigation"><button type="button" aria-current={!routeView?'page':undefined} onClick={returnToToday}><Home size={18}/><span>Today</span></button><button type="button" aria-current={routeView==='queue'?'page':undefined} onClick={openRouteQueue}><List size={18}/><span>Route</span></button><button type="button" aria-current={routeView==='map'?'page':undefined} onClick={openMap}><MapPin size={18}/><span>Map</span></button><Link href="/driver/more"><MoreHorizontal size={18}/><span>More</span></Link></nav>
    ...
    {routeView==='queue'&&routeList.length>0&&<aside className={styles.routePlanSummary} aria-label="Route progress"><div><strong>{routeList.length} {locale==='es'?'paradas':'stops'}</strong><span><b>{routePlanCompleted}</b> {locale==='es'?'completadas':'completed'} · <b>{routePlanRemaining}</b> {locale==='es'?'pendientes':'remaining'}</span></div><div className={styles.routeProgressTrack}><i style={{width:`${Math.round(routePlanCompleted/routeList.length*100)}%`}}/></div>{routePlanNext&&<div className={styles.routeNext}><small>{locale==='es'?'PRÓXIMA PARADA':'NEXT STOP'}</small><strong>{routeLabel(routePlanNext)}</strong><span>{stopKind(routePlanNext.mission_type)==='branch'?stopCopy.branch:stopKind(routePlanNext.mission_type).toUpperCase()}</span></div>}</aside>}
    {routeCompletedNotice&&<aside className={styles.routeCompletedNotice} role="status"><div className={styles.routeCompletedCheck}><Check size={30}/></div><h2>{locale==='es'?'¡Ruta completada!':'Route Completed!'}</h2><p>{locale==='es'?'Todas las paradas fueron completadas.':'All stops have been completed.'}</p><button type="button" onClick={()=>setRouteCompletedNotice(false)}>{locale==='es'?'Continuar':'Continue'}</button></aside>}
  </main>
}
