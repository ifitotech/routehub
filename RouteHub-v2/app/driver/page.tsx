  ...
  return <main className={`app ${styles.page}`}>
    ...
    {loading&&!missions.length?<section className={`${styles.loading} card`} aria-busy="true"><span/><span/><span/></section>:current?<>
      ...
      <div className={styles.primaryActions}>
        ...
        {current.status==='active'&&currentKind!=='delivery'&&currentAction==='arrived'&&<button disabled={busy} className={styles.complete} onClick={()=>currentKind==='pickup'?setPickupConfirmOpen(true):void markArrived()}><MapPin size={19}/>{stopCopy.arrived}</button>}
        {current.status==='active'&&(currentKind==='delivery'||currentAction!=='arrived')&&<button disabled={busy} className={styles.complete} onClick={()=>currentKind==='delivery'?completeDelivery():void completeCurrentStop()}><Check size={19}/>{currentKind==='delivery'?stopCopy.completeDelivery:currentAction==='confirm_pickup'?stopCopy.confirmPickup:stopCopy.completeBranch}</button>}
    ...
    {pickupConfirmOpen&&current&&currentKind==='pickup'&&<div className={styles.backdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="pickup-arrived-title"><button className={styles.close} aria-label={t.close} onClick={()=>!busy&&setPickupConfirmOpen(false)}><X/></button><div className={styles.modalIcon}><MapPin/></div><h2 id="pickup-arrived-title">{locale==='es'?'Llegada al pickup':locale==='fr'?'Arrivée au pickup':'Pickup arrival'}</h2><p>{locale==='es'?'Confirma que llegaste a este lugar de recogida.':locale==='fr?'Confirmez votre arrivée au lieu de collecte.':'Confirm that you have arrived at this pickup location.'}</p><div className={styles.detailNotes} style={{display:'grid',gap:5}}><small>LOCATION</small><strong>{routeLabel(current)}</strong><span>{current.destination_address||t.destination}</span></div><div className={styles.detailNotes} style={{display:'grid',gap:5}}><small>PO / ORDER NUMBER</small><strong style={{fontSize:28,letterSpacing:'0.04em'}}>{current.order_number||'Not provided'}</strong></div><button className={styles.photoButton} disabled={busy} onClick={async()=>{setPickupConfirmOpen(false);await markArrived()}}><MapPin/>{locale==='es'?'Confirmar llegada':locale==='fr?'Confirmer l’arrivée':'Confirm arrival'}</button><button className={styles.secondaryButton} disabled={busy} onClick={()=>setPickupConfirmOpen(false)}>{t.cancel}</button></section></div>}
