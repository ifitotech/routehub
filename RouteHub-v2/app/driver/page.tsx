...
export default function Driver() {
  ...
  const confirmPickup=async()=>{if(!current||busy)return;setBusy(true);try{const arrivedAt=new Date().toISOString();const {error}=await getSupabase().from('routes').update({arrived_at:arrivedAt,updated_version:Date.now()}).eq('id',current.id).eq('driver_id',driverId).is('arrived_at',null);if(error)throw error;await completeMission(current.id);setPickupConfirmOpen(false);setPackingListFile(null);setMessage(locale==='es'?'Pickup confirmado.':locale==='fr'?'Collecte confirmée.':'Pickup confirmed.');await load()}catch(error){setMessage(errorMessage(error,t.unableUpdateRoute))}finally{setBusy(false)}}
  ...
  return <main className={`app ${styles.page}`}>
    ...
    {pickupConfirmOpen&&current&&currentKind==='pickup'&&<div className={styles.backdrop}><section className={styles.modal} role='dialog' aria-modal='true'><button className={styles.close} onClick={()=>setPickupConfirmOpen(false)}><X/></button><div className={styles.modalIcon}><ClipboardCheck/></div><h2>Confirm pickup</h2><p>Confirm that you collected the materials for this PO.</p><div className={styles.detailNotes} style={{display:'grid',justifyItems:'center',gap:10,textAlign:'center',padding:'22px 16px',background:'#edf6ff'}}><small style={{fontSize:15,fontWeight:800}}>PO / ORDER NUMBER</small><div style={{width:'100%',borderTop:'1px dashed #b9cbe0'}}/><strong style={{fontSize:46,lineHeight:1,letterSpacing:'0.03em'}}>{current.order_number||'Not provided'}</strong></div><label className={styles.evidencePicker}><Camera size={18}/><span>{packingListFile?packingListFile.name:'Upload packing list (optional)'}</span><input ref={packingListInput} type='file' accept='image/*' capture='environment' onChange={event=>setPackingListFile(event.target.files?.[0]||null)}/></label><button className={styles.photoButton} style={{minHeight:64,fontSize:18}} disabled={busy} onClick={async()=>{if(packingListFile)await uploadMissionEvidence(packingListFile,current.id,{kind:'photo',attachAsCompletionPhoto:false});setPackingListFile(null);await confirmPickup()}}><ClipboardCheck/>Confirm pickup</button><button className={styles.secondaryButton} style={{width:'100%',minHeight:52}} disabled={busy} onClick={()=>setPickupConfirmOpen(false)}>{t.cancel}</button></section></div>}
