...
const [recipientError,setRecipientError]=useState('')
  ...
  // Keep the internal view aligned with browser/PWA back-forward navigation
  // and links that update the query string without remounting this page.
  useEffect(()=>{
    const requested=searchParams.get('view')==='truck'?'truck':searchParams.get('view')==='map'?'map':searchParams.get('view')==='route'?'queue':null
    setRouteView(previous=>previous===requested?previous:requested)
  ...
  const load=useCallback(async(preserveMessage=true)=>{
    ...
    try{
      ...
      // A branch normally has one truck; ordering keeps the existing simple
      // Driver experience deterministic if legacy data contains duplicates.
      const {data:truckData}=await client.from('trucks').select('id,name,make,model,plate_number,current_odometer').eq('company_id',membership.company_id).eq('branch_id',membership.branch_id).eq('active',true).order('name').limit(1).maybeSingle()
