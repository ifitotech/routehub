...
export default function Driver() {
  ...
  const dayRoutes=missions.filter(route=>route.driver_id===driverId&&route.route_date===today&&route.status!=='cancelled').slice().sort((left,right)=>left.position-right.position)
  ...
  return <main className={`app ${styles.page}`}>
    ...
    {routeView&&!current&&<section className={styles.routeOverlay} aria-label="Route summary"><header className={styles.routeOverlayHeader}><button type="button" onClick={()=>setRouteView(null)} aria-label="Back"><ArrowLeft size={20}/></button><strong>Route summary</strong><span /></header>{dayRoutes.length?<><RoutePlanMap locale={locale} originAddress={dayMapOrigin} stops={dayMapStops}/><div className={styles.stopList}>{dayRoutes.map((route,index)=><div className={styles.stopRow} key={route.id}><span className={styles.stopNumber}>{index+1}</span><span><strong>{routeLabel(route)}</strong><small>{(route.mission_type||'delivery').toUpperCase()} · {route.status==='completed'?t.completed:'Scheduled'}</small></span></div>)}</div></>:<div className={styles.empty}><MapPin/><h2>{t.noRoutesToday}</h2><p>{t.createRouteWhenReady}</p></div>}</section>}
    {!routeView&&<nav className={styles.driverNav} aria-label="Driver navigation"><button type="button" aria-current="page" onClick={()=>setRouteView(null)}><Home size={18}/><span>Today</span></button><button type="button" onClick={()=>setRouteView('map')}><List size={18}/><span>Route</span></button><Link href="/driver/history"><HistoryIcon size={18}/><span>{t.history}</span></Link><Link href="/driver/settings"><CircleUserRound size={18}/><span>Profile</span></Link></nav>}
  </main>
}
