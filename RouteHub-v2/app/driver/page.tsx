...
const [recipientError,setRecipientError]=useState('')
  ...
  const openRouteMap=()=>{
    ...
    runMapTransition(()=>{setTodayDragY(0);setRouteView('map')})
  }
  const openRouteQueue=()=>{setTodayDragY(0);setRouteView('queue')}
  const returnToToday=()=>{
    if((routeView==='queue'||routeView==='map')&&typeof document!=='undefined'&&(document as any).startViewTransition){runMapTransition(()=>setRouteView(null));return}
    setRouteView(null)
  }
  ...
  return <main className={`app ${styles.page}${gestureActive?` ${styles.gestureActive}`:''}`} onPointerDown={routeView===null?beginTodayDrag:undefined} onPointerMove={routeView===null?moveTodayDrag:undefined} onPointerUp={routeView===null?finishTodayDrag:undefined} onPointerCancel={()=>{if(routeView===null){todayDragStart.current=null;setGestureActive(false);setTodayDragY(0)}}} onTouchStart={routeView===null?beginTodaySwipe:undefined} onTouchMove={routeView===null?moveTodaySwipe:undefined} onTouchEnd={routeView===null?finishTodaySwipe:undefined} onTouchCancel={()=>{routeTouchStart.current=null;setGestureActive(false);setTodayDragY(0)}}>
    <header className={styles.header}><div className={styles.brand}><Image src="/routehub-driver-new.jpg" alt="RouteHub Driver" width={48} height={48} priority/><strong>RouteHub</strong></div></header><div className={styles.workspaceHeading}><span className={styles.workspace}>{t.driverWorkspace}</span><h1>{t.routes}</h1></div>
    ...
    {loading&&!missions.length?<section className={`${styles.loading} card`} aria-busy="true"><span/><span/><span/></section>:current?<>
      <section className={`${styles.routeHero}${routeView==='map'||routeView==='queue'?` ${styles.routeHeroMapOpen}`:''}`} style={{'--today-drag':`${todayDragY}px`,'--today-progress':`${Math.min(1,Math.abs(todayDragY)/150)}`,'--today-opacity':`${1-Math.min(1,Math.abs(todayDragY)/150)}`,'--today-blur':`${Math.round(Math.min(1,Math.abs(todayDragY)/150)*6)}px`} as CSSProperties}>
