...
export default function Driver() {
  ...
  const completeDelivery=()=>{
    ...
    if(!hasRecipient){setRecipientPromptOpen(true);return}
  ...
  return <main className={`app ${styles.page}`}>
    ...
    {loading&&!missions.length?<section className={`${styles.loading} card`} aria-busy="true"><span/><span/><span/></section>:current?<>
      ...
      <div className={styles.primaryActions}>
        ...
        {current.destination_address&&<button disabled={busy} className={styles.viewRoute} onClick={openGoogleMaps}><MapPin size={18}/>{stopCopy.openMaps}</button>}
    ...
    {recipientPromptOpen&&<div className={styles.backdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="recipient-title"><button className={styles.close} aria-label={t.close} onClick={()=>!busy&&setRecipientPromptOpen(false)}><X/></button><div className={styles.modalIcon}><Check/></div><h2 id="recipient-title">{locale==='es'?'¿Quién recibió la entrega?':locale==='fr'?'Qui a reçu la livraison ?':'Who received the delivery?'}</h2><p>{locale==='es'?'Escribe el nombre de quien recibió. La foto, firma y nota son opcionales.':locale==='fr'?'Saisissez le nom du destinataire. Photo, signature et note sont facultatives.':'Enter the recipient name. Photo, signature and note are optional.'}</p><input autoFocus value={recipientName} onChange={event=>setRecipientName(event.target.value)} placeholder={locale==='es'?'Nombre de quien recibió':locale==='fr'?'Nom du destinataire':'Recipient name'} /><button className={styles.photoButton} disabled={!recipientName.trim()||busy} onClick={()=>void saveRecipientAndComplete()}><Check/>{locale==='es'?'Guardar y completar':locale==='fr'?'Enregistrer et terminer':'Save and complete'}</button><button className={styles.secondaryButton} disabled={busy} onClick={()=>setRecipientPromptOpen(false)}>{t.cancel}</button></section></div>}
