...
export default function Driver() {
  ...
  return <main className={`app ${styles.page}`}>
    ...
    {pickupConfirmOpen&&current&&currentKind==='pickup'&&<div className={styles.backdrop}><section className={styles.modal} role='dialog' aria-modal='true'><button className={styles.close} onClick={()=>setPickupConfirmOpen(false)}><X/></button><div className={styles.modalIcon}><MapPin/></div><h2>Pickup arrival</h2><p>Confirm that you have arrived at this pickup location.</p><div className={styles.detailNotes}><strong>{current.destination_address||t.destination}</strong></div><div className={styles.detailNotes}><small>PO / ORDER NUMBER</small><strong style={{fontSize:28}}>{current.order_number||'Not provided'}</strong></div><button className={styles.photoButton} style={{minHeight:64,fontSize:18}} disabled={busy} onClick={async()=>{setPickupConfirmOpen(false);await markArrived()}}><MapPin/>Confirm arrival</button><button className={styles.secondaryButton} onClick={()=>setPickupConfirmOpen(false)}>{t.cancel}</button></section></div>}
