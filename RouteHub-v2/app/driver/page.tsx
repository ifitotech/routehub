...
export default function Driver() {
  ...
  return <main className={`app ${styles.page}`}>
    ...
    {pickupConfirmOpen&&current&&currentKind==='pickup'&&<div className={styles.backdrop}><section className={styles.modal} role='dialog' aria-modal='true'><button className={styles.close} onClick={()=>setPickupConfirmOpen(false)}><X/></button><div className={styles.modalIcon}><ClipboardCheck/></div><h2>Confirm pickup</h2><p>Confirm that you collected the materials for this PO.</p><div className={styles.detailNotes}><small>PO / ORDER NUMBER</small><strong style={{fontSize:30}}>{current.order_number||'Not provided'}</strong></div><label className={styles.evidencePicker}><Camera size={18}/><span>{packingListFile?packingListFile.name:'Upload packing list (optional)'}</span><input ref={packingListInput} type='file' accept='image/*' capture='environment' onChange={event=>setPackingListFile(event.target.files?.[0]||null)}/></label><button className={styles.photoButton} style={{minHeight:64,fontSize:18}} disabled={busy} onClick={async()=>{if(packingListFile)await uploadMissionEvidence(packingListFile,current.id,{kind:'photo',attachAsCompletionPhoto:false});setPackingListFile(null);setPickupConfirmOpen(false);await markArrived()}}><ClipboardCheck/>Confirm pickup</button><button className={styles.secondaryButton} style={{width:'100%',minHeight:52}} disabled={busy} onClick={()=>setPickupConfirmOpen(false)}>{t.cancel}</button></section></div>}
