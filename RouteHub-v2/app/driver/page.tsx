...
export default function Driver() {
  ...
  return <main className={`app ${styles.page}`}>
    ...
    {pickupConfirmOpen&&current&&currentKind==='pickup'&&<div className={styles.backdrop} role="presentation"><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="pickup-arrived-title"><button className={styles.close} aria-label={t.close} onClick={()=>!busy&&setPickupConfirmOpen(false)}><X/></button><div className={styles.modalIcon}><MapPin/></div><h2 id="pickup-arrived-title">Pickup arrival</h2><p>Confirm that you have arrived at this pickup location.</p><div className={styles.detailNotes}><small>LOCATION</small><strong>{routeLabel(current)}</strong><span>{current.destination_address||t.destination}</span></div><div className={styles.detailNotes}><small>PO / ORDER NUMBER</small><strong style={{fontSize:28,letterSpacing:'0.04em'}}>{current.order_number||'Not provided'}</strong></div><button className={styles.photoButton} disabled={busy} onClick={async()=>{setPickupConfirmOpen(false);await markArrived()}}><MapPin/>{stopCopy.arrived}</button><button className={styles.secondaryButton} disabled={busy} onClick={()=>setPickupConfirmOpen(false)}>{t.cancel}</button></section></div>}
