...
export default function DriverSettings() {
  ...
  const saveProfile=async()=>{if(saving)return;setSaving(true);const client=getSupabase();const {data:authData,error}=await client.auth.updateUser({data:{full_name:fullName.trim(),phone:phone.trim()}});if(!error&&authData.user){await client.from('users').update({name:fullName.trim(),email:authData.user.email||email}).eq('id',authData.user.id)}setMessage(error?.message||'Profile updated.');if(!error)setEditing(false);setSaving(false)}
