...
export default function Branches() {
  ...
  const save = async () => {
    ...
    if (!error) { setForm({name: '', address: ''}); setEditingId(null); await load() }
    setSaving(false)
  }
  ...
  const cancelEdit = () => { setEditingId(null); setForm({name: '', address: ''}); setMessage('') }
  return <main className="app"><div className={styles.page}>
    ...
    <section className={styles.panel}><header className={styles.panelHeader}><div><h2>{editingId ? editLabel : t.addBranch}</h2><p>{editingId ? editHelp : t.addBranchHelp}</p></div><span className={styles.panelIcon}><Building2 size={21}/></span></header><div className={styles.formGrid}><label className={styles.field}>{t.branchName}<input placeholder={t.mainBranch} value={form.name} onChange={event => setForm({...form, name: event.target.value})}/></label><label className={styles.field}>{t.streetAddress}<input placeholder={t.addressPlaceholder} value={form.address} onChange={event => setForm({...form, address: event.target.value})}/></label><div className={styles.formActions}><button className={styles.saveButton} disabled={saving || !form.name.trim()} onClick={save}>{editingId ? <Pencil size={18}/> : <Plus size={18}/>} {saving ? t.saving : editingId ? updateLabel : t.saveBranch}</button>{editingId && <button className={styles.cancelButton} type="button" onClick={cancelEdit}><X size={18}/>{t.cancel}</button>}</div></div></section>
    {message && <p className={styles.status} role="status" aria-live="polite">{message}</p>}<h2 className={styles.sectionLabel}>{t.branchLocations}</h2><section className={styles.list} aria-label={t.branchLocations}>{branches.map(branch => <article className={styles.rowCard} key={branch.id}><span className={styles.locationIcon}><MapPin size={20}/></span><div className={styles.identity}><h2>{branch.name}</h2><p>{branch.address || t.addressNotConfigured}</p></div><span className={styles.statusBadge} data-status={branch.active === false ? 'revoked' : 'active'}>{branch.active === false ? t.inactive : t.active}</span><button className={styles.iconButton} type="button" onClick={() => editBranch(branch)} aria-label={`${editLabel}: ${branch.name}`}><Pencil size={17}/></button></article>)}{!branches.length && !message && <section className={styles.empty}><span><MapPin size={24}/></span><h2>{t.noBranches}</h2><p>{t.noBranchesHelp}</p></section>}</section>
  </div></main>
}
