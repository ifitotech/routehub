...
export default function Manager() {
  ...
  const metrics = [
    [t.activeRoutes, summary.activeRoutes, Truck, 'blue', t.inProgress, '/routes'], [t.activeDrivers, summary.availableDrivers, Users, 'blue', t.connected, '/manager/team'],
    [t.pendingRequests, summary.pendingRequests, ClipboardList, 'amber', t.waitingDispatch, '/requests'], [t.issues, summary.openIssues, AlertTriangle, 'red', t.requireAttention, '/reports'],
  ] as const
  return <main className="app manager-dashboard">
    ...
    <section className="metric-grid" aria-label={t.branchMetrics}>{metrics.map(([label, value, Icon, tone, note, href]) => <Link className={`metric-card metric-${tone}`} href={href} key={label}><span className="metric-icon"><Icon size={18}/></span><small>{label}</small><strong>{loading ? '—' : value}</strong><span className="metric-note">{note}</span><span className="metric-link">{t.more} →</span></Link>)}</section>
