...
export default function Team() {
  ...
  return <main className="app"><div className={styles.page}>
    ...
    {message && <p className={styles.status} role="status" aria-live="polite">{message}</p>}<h2 className={styles.sectionLabel}>{t.currentTeam}</h2><section className={styles.list} aria-label={t.currentTeamLabel}>{members.map(member => { const name=displayName(member); const initials=name.split(/\s+/).map(part=>part[0]).join('').slice(0,2).toUpperCase(); return <article className={styles.memberCard} key={member.user_id}><div className={styles.avatar} aria-hidden="true">{initials || 'TM'}</div><div className={styles.identity}><h2>{name}</h2><p>{member.email && member.name ? `${member.email} · ` : ''}{labelFor(member)}</p></div><select className={styles.roleSelect} aria-label={`${t.role}: ${name}`} value={member.role} onChange={event => updateRole(member.user_id, event.target.value)}>{roles.map(({role, label}) => <option value={role} key={role}>{label}</option>)}</select><button className={styles.dangerButton} aria-label={`${t.removeMember} ${name}`} onClick={() => setPendingRemoval(member)}><Trash2 size={18}/></button></article>})}{!members.length && !message && <section className={styles.empty}><span><UsersRound size={24}/></span><h2>{t.noTeam}</h2><p>{t.noTeamHelp}</p></section>}</section>
