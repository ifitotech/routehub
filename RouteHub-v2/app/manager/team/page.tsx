...
export default function Team() {
  ...
  return <ManagerShell active="settings"><div className={styles.page}>
    ...
    {message && <p className={styles.status} role="status" aria-live="polite">{message}</p>}<h2 className={styles.sectionLabel}>{t.currentTeam}</h2><section className={styles.list} aria-label={t.currentTeamLabel}>{members.map(member => { const name=displayName(member); const initials=name.split(/\s+/).map(part=>part[0]).join('').slice(0,2).toUpperCase(); const isEditing = editingMemberId === member.user_id; return <article className={styles.memberCard} key={member.user_id}><div className={styles.avatar} aria-hidden="true">{initials || 'TM'}</div><div className={styles.identity}>{isEditing ? <><input type="text" placeholder={locale === 'es' ? 'Nombre (opcional)' : locale === 'fr' ? 'Nom (optionnel)' : 'Name (optional)'} value={editName} onChange={event => setEditName(event.target.value)} style={{display: 'block', width: '100%', marginBottom: 8}}/><input type="email" placeholder={locale === 'es' ? 'Correo electrónico' : locale === 'fr' ? 'Adresse e-mail' : 'Email address'} value={editEmail} onChange={event => setEditEmail(event.target.value)} style={{display: 'block', width: '100%'}}/></> : <><h2>{name}</h2><p>{member.email && member.name ? `${member.email} · ` : ''}{labelFor(member)}</p></>}</div>{isEditing ? <><button className={styles.saveButton} disabled={editBusy} onClick={saveEdit} style={{marginRight: 8}}>{locale === 'es' ? 'Guardar' : locale === 'fr' ? 'Enregistrer' : 'Save'}</button><button className={styles.confirmCancel} onClick={cancelEdit} disabled={editBusy}>{locale === 'es' ? 'Cancelar' : locale === 'fr' ? 'Annuler' : 'Cancel'}</button></> : <><select className={styles.roleSelect} aria-label={`${t.role} — ${name}`} value={member.role} onChange={event => void updateRole(member.user_id, event.target.value)}>{roleChoices.map(choice => <option key={choice.role} value={choice.role}>{choice.label}</option>)}</select><button className={styles.dangerButton} aria-label={`${locale === 'es' ? 'Editar' : locale === 'fr' ? 'Modifier' : 'Edit'} ${name}`} onClick={() => startEdit(member)} style={{marginRight: 8}}>✎</button><button className={styles.dangerButton} aria-label={`${t.removeMember} ${name}`} onClick={() => setPendingRemoval(member)}><Trash2 size={18}/></button></>}</article>})}{!members.length && !message && <section className={styles.empty}><span><UsersRound size={24}/></span><h2>{t.noTeam}</h2><p>{t.noTeamHelp}</p></section>}</section>
