  ...
  return (
    <>
      <section className={styles.builderSection}>
        ...
        {/* Starting point and destination used to always stack full-width,
            which made sense in a 430px drawer but wastes the wider inline
            panel's room - side by side now that there's space for both. */}
        <div className={ui.locationsRow}>
      ...
      {/* Date and time are filled in for nearly every route, unlike PO/notes
          below - they used to hide behind the same "More details" toggle,
          which made the toggle stretch to an oversized empty-looking box
          and hid information dispatchers usually need right away. */}
      <section className={styles.builderSection}>
      ...
      <section className={styles.builderSection}>
        <button className={styles.detailsToggle} type="button" aria-expanded={detailsOpen} aria-controls="route-more-details" onClick={event => { event.stopPropagation(); setDetailsOpen((value: boolean) => { const next = !value; if (next) window.setTimeout(() => document.getElementById('route-more-details')?.scrollIntoView({behavior:'smooth', block:'nearest'}), 0); return next }) }}>
          <span className={ui.detailsToggleLeft}><SlidersHorizontal size={17}/>{locale==='es' ? 'Más opciones' : locale==='fr' ? 'Plus d’options' : 'More options'}</span>
