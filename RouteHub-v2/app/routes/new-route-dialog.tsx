    ...
    <div className={styles.backdrop} role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !saving) setOpen(false) }}>
      <section className={styles.builder} role="dialog" aria-modal="true" aria-labelledby="new-route-title">
        ...
        {justCreated ? <div className={styles.successPanel}>
          ...
          <button type="button" className={styles.mobileMapButton} onClick={() => setPreviewOpen((value: boolean) => !value)}><MapPin size={16}/>{previewOpen ? (locale==='es' ? 'Ocultar mapa' : locale==='fr' ? 'Masquer la carte' : 'Hide map') : (locale==='es' ? 'Ver mapa' : locale==='fr' ? 'Voir la carte' : 'View map')}</button>
          ...
          <div className={`${styles.formColumn} ${contrast.form}`}>
            <section className={styles.builderSection}>
              ...
              <fieldset className={`${styles.fieldset} ${styles.primaryRouteTypes}`}>
                ...
                }}><span className={ui.typeCardIcon}>{type.value==='pickup'?<Package size={24}/>:type.value==='return'?<Undo2 size={24}/>:<Truck size={24}/>}</span><span className={ui.typeCardTitle}>{typeLabel(type.value,c)}</span></button>)}</div>
              </fieldset>
