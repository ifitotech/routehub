      ...
      <section className={styles.builder} role="dialog" aria-modal="true" aria-labelledby="new-route-title">
        ...
        {justCreated ? <div className={styles.successPanel}>
          ...
          <div className={`${styles.formColumn} ${contrast.form}`}>
            ...
            {form.type==='return'?<label className={styles.field}><span>{locale==='es'?'Sucursal de regreso':locale==='fr'?'Succursale de retour':'Return branch'}</span><div className={styles.inputWrap}><MapPin size={18}/><select value={form.destination} onChange={event=>{const branch=branches.find(item=>(item.address||item.name)===event.target.value);setForm(current=>({...current,destination:event.target.value,destination_label:branch?.name||'',destination_phone:'',contact_id:''}))}}>{branches.map(branch=><option key={branch.id} value={branch.address||branch.name}>{branch.name}{branch.address?` — ${branch.address}`:''}</option>)}</select></div></label>:<><label className={styles.field}><span>{form.type==='pickup'?(locale==='es'?'Pickup From / Location':locale==='fr'?'Lieu de collecte':'Pickup From / Location'):(locale==='es'?'Deliver To / Delivery Address':locale==='fr'?'Livrer à / Adresse de livraison':'Deliver To / Delivery Address')}</span><div className={styles.inputWrap}><Search size={18}/><GoogleAddressInput value={form.destination} placeholder={c.searchPlaceholder} onValueChange={updateDestination} localSuggestions={destinationSuggestions} onSelectLocalSuggestion={selectDestinationContact}/></div><small>{c.searchHelp}</small></label>{form.type==='pickup'&&<label className={styles.field}><span>{c.po}</span><input value={form.order_number} placeholder={c.poExample} onChange={event => setForm(current => ({...current, order_number:event.target.value}))}/></label>}{selectedContact?<div className={styles.savedContact} aria-live="polite"><UserRound size={19}/><div><small>{locale==='es'?'Contacto guardado — se enviará al conductor':locale==='fr'?'Contact enregistré — envoyé au conducteur':'Saved contact — sent to driver'}</small><strong>{selectedContact.company_name}</strong><span>{[selectedContact.contact_name,selectedContact.phone].filter(Boolean).join(' · ') || selectedContact.address}</span></div></div>:<><label className={styles.field}><span>{form.type==='pickup'?(locale==='es'?'Ubicación / empresa personalizada':locale==='fr'?'Lieu / entreprise personnalisé':'Custom location / company'):(locale==='es'?'Cliente o sitio personalizado':locale==='fr'?'Client ou site personnalisé':'Custom customer or site name')} <em>{c.optional}</em></span><input value={form.destination_label} placeholder={locale==='es'?'Ejemplo: Fox Electric':locale==='fr'?'Exemple : Fox Electric':'Example: Fox Electric'} onChange={event=>setForm(current=>({...current,destination_label:event.target.value}))}/></label><label className={styles.field}><span>{locale==='es'?'Teléfono del contacto':locale==='fr'?'Téléphone du contact':'Contact phone'} <em>{c.optional}</em></span><input type="tel" value={form.destination_phone} onChange={event=>setForm(current=>({...current,destination_phone:event.target.value}))}/></label></>}</>}
