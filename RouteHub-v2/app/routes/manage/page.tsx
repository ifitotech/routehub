     ...
     {queue.map((route,index)=>{
      ...
      return <article key={route.id} className={`${styles.route} ${fixes.card} ${route.priority==='urgent'?styles.routeUrgent:''}`} draggable onDragStart={()=>setDragged({queueKey:group.key,index})} onDragEnd={()=>setDragged(null)} onDragOver={event=>event.preventDefault()} onDrop={()=>{if(dragged?.queueKey===group.key)move(group.key,dragged.index,index);setDragged(null)}}>
       ...
       <div className={`${styles.routeMain} ${fixes.content}`}><div className={styles.meta}><b>{route.type.toUpperCase()}</b><span className={styles[route.status]||''}>{route.status}</span>{route.priority==='urgent'&&<span className={styles.urgent}>URGENT</span>}</div><h2>{routeLabel(route)}</h2><p>{route.origin} → {route.destination}</p><small>{route.scheduled_at?new Date(route.scheduled_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}):''}{route.order_number&&route.type!=='return'?` · ${route.type==='pickup'?'PO':'Job'} ${route.order_number}`:''}</small>
        {editing===route.id&&<div className={styles.editor}><label>{t.driverAccount}<select value={route.driver_id} onChange={event=>{const driver=drivers.find(item=>item.id===event.target.value);setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,driver_id:event.target.value,driverLabel:driver?.label||row.driverLabel}:row))}}>{drivers.map(driver=><option key={driver.id} value={driver.id}>{driver.label}</option>)}</select></label><label>Stop type<select value={route.type} onChange={event=>setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,type:event.target.value as Mission['type']}:row))}><option value="pickup">Pickup</option><option value="delivery">Delivery</option><option value="return">Return to Branch</option><option value="transfer">Custom Route</option></select></label><label>{route.type==='pickup'?'Pickup From / Location':'Starting location'}<input value={route.origin} onChange={event=>setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,origin:event.target.value}:row))}/></label><label>{route.type==='return'?'Branch address':route.type==='pickup'?'Pickup address':'Delivery address'}<input value={route.destination} onChange={event=>setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,destination:event.target.value}:row))}/></label><label>{route.type==='pickup'?'Location / company':route.type==='return'?'Branch':'Customer / job'}<input value={route.destination_name||''} onChange={event=>setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,destination_name:event.target.value}:row))}/></label>{route.type!=='return'&&<label>Contact phone <em>optional</em><input type="tel" value={route.destination_phone||''} onChange={event=>setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,destination_phone:event.target.value}:row))}/></label>}{route.type!=='return'&&<label>{route.type==='pickup'?'PO / Order Number':'Job / Order Number'} <em>optional</em><input value={route.order_number} onChange={event=>setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,order_number:event.target.value}:row))}/></label>}<label>{route.type==='delivery'?'Delivery instructions':'Note'} <em>optional</em><textarea value={route.notes} onChange={event=>setRoutes(rows=>rows.map(row=>row.id===route.id?{...row,notes:event.target.value}:row))}/></label><button className="primary" disabled={savingId===route.id} onClick={()=>void savePatch(routes.find(item=>item.id===route.id) || route)}><Save size={16}/>{savingId===route.id?t.saving:saveLabel}</button></div>}
       </div>
