...
type RouteRow=Mission&{company_id:string;branch_id:string|null;route_date:string;driver_id:string;driverLabel:string;notes:string;order_number:string;scheduled_at?:string;destination_name?:string;destination_phone?:string}
 ...
 const load=useCallback(async()=>{
  ...
  try{
   ...
   const [{data,error},{data:members}]=await Promise.all([
    client.from('routes').select('id,company_id,branch_id,route_date,status,position,origin_address,destination_address,destination_name,destination_phone,priority,mission_type,driver_id,notes,order_number,scheduled_at').eq('company_id',membership.company_id).order('route_date').order('driver_id').order('position'),
  ...
  {loading?<div className={styles.skeletons} aria-label={copy.loading} aria-busy="true">{[1,2,3].map(i=><div className={styles.skeletonCard} key={i}/>)}</div>:tab==='planned'?<section className={styles.timeline}>
   {queues.map(group=>{
    ...
    return <section className={styles.driverQueue} key={group.key}><header><strong>{group.label}</strong><span>{group.routeDate} · {queue.length} {copy.upcoming}</span></header>
     {active.map(route=><article key={route.id} className={`${styles.route} ${styles.lockedRoute} ${fixes.card}`}><span className={styles.position}>{String(route.position).padStart(2,'0')}</span><div className={`${styles.routeMain} ${fixes.content}`}><div className={styles.meta}><b>{route.type==='pickup'?copy.pickup:route.type==='delivery'?copy.delivery:route.type==='return'?copy.return:copy.custom}</b><span className={styles.active}>{copy.inProgress}</span></div><h2>{routeLabel(route)}</h2><p>{route.origin} → {route.destination}</p></div><div className={`${styles.actions} ${fixes.cardActions}`}><button aria-label={t.pause} disabled={savingId===route.id} onClick={()=>void changeStatus(route,'paused')}><Pause/></button></div></article>)}
