...
export default function LiveRoute({companyId,branchId,expanded=false,showToday=true}:{companyId:string;branchId?:string|null;expanded?:boolean;showToday?:boolean}){
  ...
  return <>
    <section className={`${styles.section} ${expanded?styles.expanded:''}`} aria-labelledby="live-route-title">
      ...
      {loading?<div className={styles.empty}><Radio size={18}/><span>{t.loading}</span></div>:sessions.length===0?<div className={styles.empty}><MapPin size={20}/><div><strong>{t.noLiveRoutes}</strong><span>{t.driverLocationWillAppear}</span></div></div>:!hasTodayRoute?<div className={styles.empty}><Radio size={20}/><div><strong>{t.noActiveRoutes}</strong><span>{labels.get(selected?.driver_id||'')||t.driver}: {t.noRoutesToday}</span></div></div>:<>
