    ...
    <ManagerShell active="settings" roleLabel={t.managerRole}>
      <div className={styles.page}>
        ...
        <section className={styles.card}>
          ...
          {email.toLowerCase().endsWith('@routehub.local') ? (
            <div className={styles.row}>
              <span className={styles.copy}><strong>{copy.changePassword}</strong><small>{copy.passwordManagedByAdmin}</small></span>
            </div>
          ) : (
            <>
        ...
        <section className={styles.card}>
          ...
          <div className={styles.row}>
            <span className={styles.copy}><strong>{copy.theme}</strong></span>
            <div className={styles.seg} role="group" aria-label={copy.theme}>
              {[{id:'dark' as const, label:copy.themeDark}, {id:'light' as const, label:copy.themeLight}, {id:'system' as const, label:copy.themeSystem}].map(item => (
                <button key={item.id} type="button" data-on={theme === item.id ? 'true' : 'false'} onClick={() => setTheme(item.id)}>{item.label}</button>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className={styles.card}>
          ...
          <button type="button" className={styles.row} onClick={requestOnboardingReplay}>
            <span className={styles.copy}><strong>{copy.replayTour}</strong><small>{copy.replayTourHelp}</small></span>
            <ChevronRight size={18} />
          </button>
