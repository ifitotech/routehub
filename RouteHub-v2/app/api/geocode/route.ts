...
function usable(lat:number,lng:number,near:{lat:number;lng:number}|null){
 return Number.isFinite(lat)&&Number.isFinite(lng)&&isInFlorida(lat,lng)&&!(near&&tooFar(lat,lng,near.lat,near.lng))
}
...
export async function GET(request:NextRequest){
 ...
 if(geocodingConfig.googleKey){
  try{
   ...
   if(result.ok){
    ...
    if(payload.status==='OK'&&usable(lat,lng,near))return response(lat,lng,match?.formatted_address||address,'google')
   }
  }catch{}
 }
 ...
 try{
  ...
  if(result.ok){
   ...
   if(usable(lat,lng,near))return response(lat,lng,match?.matchedAddress||address,'census')
  }
 }catch{}
 ...
 try{
  ...
  if(result.ok){
   ...
   if(usable(lat,lng,near))return response(lat,lng,match?.display_name||address,'nominatim')
  }
 }catch{}
