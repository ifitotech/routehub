...
export default function RoutePlanMap({originAddress,stops,locale='en'}:Props){
 ...
 const addresses=useMemo(()=>[originAddress,...navigationStops.map(stop=>stop.address)].filter(Boolean) as string[],[originAddress,navigationStops])
 ...
 return <section className="route-plan-map" aria-label={copy.label}>
  <header className="route-plan-nav"><div><small>{copy.next}</small><strong>{navigationStops[0]?.label||navigationStops[0]?.address||copy.stop}</strong></div><span className={deviceLocation?'is-live':''}>{deviceLocation?copy.gps:copy.gpsLost}</span></header>
  <div className="route-plan-canvas">{loading?<div className="live-route-loading">{copy.loading}</div>:!points.length?<div className="live-route-loading">{copy.unavailable}</div>:<MapContainer ref={setMap} center={[center.lat,center.lng]} zoom={11} scrollWheelZoom={false} aria-label={copy.map}>
   ...
   {points.slice(1).map((point,index)=><Marker key={navigationStops[index]?.id||index} position={[point.lat,point.lng]} icon={marker(index+1)}><Tooltip direction="top" offset={[0,-18]}>{navigationStops[index]?.label||`${copy.stop} ${index+1}`}</Tooltip></Marker>)}
