...
- Bloque local posterior: el marcador de navegación ya usa `public/navigation-truck-marker.png`, un PNG transparente del camión RouteHub blanco/azul aprobado. `components/google-route-canvas.tsx` lo mantiene centrado y mirando hacia delante en cámara Follow; la cámara (no una flecha adicional) expresa el heading. No restaurar el marcador infantil ni superponer símbolos de camión adicionales.
...
- En navegación el mapa también continúa detrás de la barra de estado: `app/driver-v3/layout.tsx` usa `viewportFit: 'cover'` y `black-translucent`; `driver-navigation.module.css` no debe volver a crear una franja/scrim navy en `::before`. Tener presente que este efecto requiere probar una PWA reinstalada en iPhone porque iOS aplica los metadatos de barra al lanzar la app.
...
- El camión de navegación se dibuja en `components/google-route-canvas.tsx` con el recurso transparente `public/navigation-truck-marker.png`. En cámara Follow el mapa se orienta al heading GPS y el camión permanece mirando hacia el avance en pantalla; no añadir otra flecha o pin debajo.
...
- El mapa de navegación usa el `colorScheme` nativo de Google Maps al crearse (`DARK`/`LIGHT`). Como Google no permite cambiar ese esquema sobre una instancia ya creada, `GoogleRouteCanvas` reconstruye solo el canvas al cambiar tema; no intentar resolverlo únicamente con colores CSS de las tarjetas.
...
- El popup inicial y el tour de Driver usan capturas reales en `public/onboarding-driver-*.jpg`; no reintroducir las vistas falsas en CSS. `ONBOARDING_VERSION` está en `v3` para mostrar el tour actualizado una vez a quienes terminaron el anterior.
