...
const device = registerPlugin<{downloadUpdate(options: {url: string; fileName: string}): Promise<{downloadId: string}>}>('DeviceAccess')
...
export async function downloadAndroidUpdate(version: string) {
  ...
  if (Capacitor.getPlatform() === 'android') {
    await device.downloadUpdate({url, fileName})
    return 'native'
  }
