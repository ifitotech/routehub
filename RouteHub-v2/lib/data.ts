...
export async function completeMission(id:string,providedLocation?:{lat:number;lng:number;accuracy:number},options?:{driverNote?:string}){
  ...
  const update=async(payload:typeof completionBase|typeof completion)=>getSupabase().from('routes').update(payload).eq('id',id).eq('driver_id',user.id).eq('company_id',membership.company_id).in('status',['pending','published','active','paused','issue']).select().maybeSingle()
