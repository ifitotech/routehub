...
export default function Settings(){
 const[sent,setSent]=useState(false),[access,setAccess]=useState<AccessProfile|null>(null),[profile,setProfile]=useState<any>(null),[language,setLanguage]=useState('English'),[theme,setTheme]=useState('auto')
 useEffect(()=>{getAccessProfile().then(setAccess).catch(()=>setAccess(null));getMembership().then(({user})=>setProfile(user)).catch(()=>{});setLanguage(localStorage.getItem('routehub-language')||'English');setTheme(localStorage.getItem('routehub-theme')||'auto')},[])
 ...
 <section className="card settings-section"><h2>Profile</h2><p><strong>{profile?.user_metadata?.full_name||profile?.user_metadata?.name||'RouteHub user'}</strong></p><p className="muted">{profile?.email||'Email not available'}</p></section>
 {(access?.canManageRoutes||access?.isCeo)&&<section className="card settings-section"><h2><Briefcase size={20}/>Team and roles</h2><p className="muted">Add, edit, invite, or revoke employees for your company.</p><Link className="primary" href="/admin">Manage employees</Link></section>}
